...
function DeleteCollection({ onDeleteComplete, totalAlumnos }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        setMessage(`Colección borrada exitosamente. ${result.deletedCount} alumnos eliminados.`);
  ...
  return (
    <div className="bg-white p-10 rounded-2xl shadow-xl border border-gray-200 w-full max-w-xl mx-auto">
      ...
      {!showConfirm ? (
        <div>
          <p className="mb-4 text-lg text-gray-700">Esta acción eliminará permanentemente todos los <span className="font-bold">{totalAlumnos}</span> alumnos de la base de datos.</p>
          <p className="text-red-600 text-base mb-6 font-semibold">⚠️ Esta acción no se puede deshacer.</p>
          <button
        ...
        <div>
          <p className="text-red-700 font-bold mb-4 text-lg">¿Estás seguro de que quieres borrar todos los {totalAlumnos} alumnos?</p>
          <p className="text-gray-700 mb-6 text-base">Esta acción eliminará permanentemente todos los datos y no se puede deshacer.</p>
          {message && (
            <div className={`p-4 rounded-md mb-6 text-lg ${message.includes('Error') ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>{message}</div>
          )}
