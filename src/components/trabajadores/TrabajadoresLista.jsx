...
const TrabajadoresLista = ({
  ...
  const trabajadoresFiltrados = useMemo(() => {
    return trabajadores.filter(trabajador => {
      ...
      const cumpleRut =
        !rutFiltro || trabajador.rut.toLowerCase().startsWith(rutFiltro);
      const cumpleNombres =
        !localNombres ||
        (trabajador.nombres ?? trabajador.nombre ?? '')
          .toLowerCase()
          .includes(localNombres.toLowerCase());
      const cumpleApellidos =
        !localApellidos ||
        (
          ...
          .toLowerCase()
          .includes(localApellidos.toLowerCase());
      const cumpleObservacion =
        !localObservacion ||
        (trabajador.observacion ?? '')
          .toLowerCase()
          .includes(localObservacion.toLowerCase());
  ...
  const totalPaginas = Math.max(
    1,
    Math.ceil(trabajadoresOrdenados.length / pageSize)
  );
                ...
                {trabajadoresOrdenados.length === 0 ? (
                  ...
                  trabajadoresPagina.map((trabajador, idx) => (
                    ...
                    >
                      ...
                      {columnasVisibles.rut && (
                        <td className='py-4 px-4 font-mono text-slate-700 min-w-32'>
                          {trabajador.rut}
                        </td>
                      )}
                      {columnasVisibles.nombres && (
            ...
            <div className='flex items-center justify-between px-4 py-3 border-t border-slate-200 bg-slate-50 text-sm text-slate-700'>
              <div>
                ...
                {Math.min(
                  ...
                  trabajadoresOrdenados.length
                )}{' '}
                de {trabajadoresOrdenados.length} funcionarios
              </div>
              <div className='flex items-center gap-2'>
                <button
