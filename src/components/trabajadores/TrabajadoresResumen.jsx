...
function TrabajadoresResumen({ trabajadores, soloPresentes, setSoloPresentes, trabajadoresCompletos, eventoActivo }) {
  // Las estadísticas siempre se calculan con TODOS los trabajadores (trabajadoresCompletos)
  // para que los números no cambien al filtrar por estado
  // El prop 'trabajadores' se usa solo para determinar el estado activo de los botones
  ...
  const pendientesConfirmacion = datosParaEstadisticas.filter(t => !t.asiste).length;
  const porcentajeConfirmacion = total > 0 ? Math.round((confirmados / total) * 100) : 0;

  // Función para exportar con filtro
  const handleExportarFiltrado = (filtro) => {
    if (eventoActivo && trabajadoresCompletos) {
      exportarAExcel(trabajadoresCompletos, eventoActivo.nombre, eventoActivo.tipo, filtro);
    }
  };
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-lg font-semibold text-slate-800">Estadísticas de Asistencia y Confirmación</h3>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4 items-stretch">
        <div className="relative">
          <motion.button
            ...
            className={`p-3 rounded-lg border transition-all duration-200 w-full min-h-20 flex flex-col justify-center ${soloPresentes === ""
        ...
        <div className="relative">
          <motion.button
            ...
            className={`p-3 rounded-lg border transition-all duration-200 w-full min-h-20 flex flex-col justify-center ${soloPresentes === "presentes"
        ...
        <div className="relative">
          <motion.button
            ...
            className={`p-3 rounded-lg border transition-all duration-200 w-full min-h-20 flex flex-col justify-center ${soloPresentes === "ausentes"
        ...
        <div className="relative">
          <motion.button
            ...
            className={`p-3 rounded-lg border transition-all duration-200 w-full min-h-20 flex flex-col justify-center ${soloPresentes === "confirmados"
          ...
          >
            <div className="text-center">
              <div className="text-sm text-slate-600 mb-1">Confirmados</div>
              <div className="text-2xl font-bold text-indigo-600">{confirmados}</div>
            </div>
          </motion.button>
          <motion.button
        ...
        <div className="relative">
          <motion.button
            onClick={() => setSoloPresentes && setSoloPresentes("pendientes")}
            className={`p-3 rounded-lg border transition-all duration-200 w-full min-h-20 flex flex-col justify-center ${soloPresentes === "pendientes"
          ...
          >
            <div className="text-center">
              ...
              <div className="text-2xl font-bold text-orange-600">{pendientesConfirmacion}</div>
            </div>
          </motion.button>
          <motion.button
