...
const AlumnosLista = ({
  ...
  const handleConfirmarEliminar = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteAlumno(alumnoAEliminar.eventoId, alumnoAEliminar.id);
      setAlumnoAEliminar(null);
    } catch (error) {
      console.error('Error al eliminar alumno:', error);
      alert('Error al eliminar el participante: ' + (error.message || 'Error desconocido'));
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const columnasConDatos = useMemo(() => {
    ...
    const tieneCamposNuevos = alumnosNormalizados.some(
      a =>
    ...
    return {
      ...
      nombreCompleto: true,
      carrera: alumnosNormalizados.some(a => a.carreraNormalizada != null && String(a.carreraNormalizada).trim() !== '' && a.carreraNormalizada !== 'General'),
      nombres: !tieneCamposNuevos && alumnosNormalizados.some(a => a.nombres != null && String(a.nombres).trim() !== ''),
  ...
  const alumnosFiltrados = useMemo(() => {
    return alumnosNormalizados.filter(alumno => {
      ...
      const cumpleBusqueda = !busqueda ||
        ...
        (alumno.cargo && alumno.cargo.toLowerCase().includes(searchLower)) ||
    ...
    <div className="w-full">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4 mb-6">
        ...
        <div className="flex items-center gap-2.5 sm:gap-4 p-3 sm:p-4 bg-white rounded-2xl border border-slate-100 shadow-sm">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-st-pastel text-st-verde rounded-xl flex items-center justify-center shrink-0">
            <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <table className="w-full text-sm">
            <thead className="bg-st-verde text-white">
              <tr>
                ...
                {columnasVisibles.establecimiento && <th className="py-3 px-4 text-left font-semibold">Establecimiento</th>}
                {columnasVisibles.cargo && <th className="py-3 px-4 text-left font-semibold">Cargo</th>}
                {columnasVisibles.comuna && <th className="py-3 px-4 text-left font-semibold">Comuna del Establecimiento</th>}
                  ...
                  return (
                    <tr key={uniqueKey} className={`${idx % 2 === 0 ? 'bg-slate-50/50' : 'bg-white'} hover:bg-green-50/50 transition-colors border-b border-slate-100`}>
                      ...
                      {columnasVisibles.telefono && (
                        <td className="py-3 px-4 text-slate-600 text-xs font-mono">{alumno.telefono ?? '-'}</td>
                      )}
                      {columnasVisibles.correo && (
                        <td className="py-3 px-4 text-slate-600 text-xs">
                          <div className="truncate max-w-[170px]" title={alumno.correo ?? '-'}>{alumno.correo ?? '-'}</div>
                        </td>
                      )}
                      {columnasVisibles.establecimiento && (
                        <td className="py-3 px-4 text-slate-600 text-xs">
                          <div className="truncate max-w-[150px]" title={alumno.establecimiento ?? alumno.institucion ?? '-'}>{alumno.establecimiento ?? alumno.institucion ?? '-'}</div>
                        </td>
                      )}
                      {columnasVisibles.cargo && (
                        <td className="py-3 px-4 text-slate-600 text-xs">
                          <div className="truncate max-w-[130px]" title={alumno.cargo ?? '-'}>{alumno.cargo ?? '-'}</div>
                        </td>
                      )}
                      {columnasVisibles.comuna && (
                        <td className="py-3 px-4 text-slate-600 text-xs">
                          <div className="truncate max-w-[130px]" title={alumno.comuna ?? '-'}>{alumno.comuna ?? '-'}</div>
                        </td>
                      )}
                      ...
                      {columnasVisibles.institucion && !columnasVisibles.establecimiento && (
                        <td className="py-3 px-4 text-slate-600 text-xs">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-100">
            ...
            <div className="bg-red-600 px-6 py-4 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold leading-tight">¿Eliminar Participante?</h3>
                  <p className="text-red-100 text-xs mt-0.5">Esta acción no se puede deshacer</p>
                </div>
              </div>
              <button
                onClick={() => !isDeleting && setAlumnoAEliminar(null)}
            ...
            <div className="p-6">
              <p className="text-sm text-slate-600 mb-3">
                ¿Estás seguro de que deseas eliminar permanentemente a este participante?
              </p>
              ...
              <div className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 mb-4">
                <p className="font-bold text-slate-800 text-base leading-snug break-words">
                  {alumnoAEliminar.nombre || `${alumnoAEliminar.nombres || ''} ${alumnoAEliminar.apellidos || ''}`.trim() || 'Sin Nombre'}
                </p>
                <p className="text-xs text-slate-500 mt-1">
                  RUT: <span className="font-semibold text-slate-700">{alumnoAEliminar.rut || '-'}</span>
                  {alumnoAEliminar.carrera && ` · ${alumnoAEliminar.carrera}`}
                </p>
              </div>

              <p className="text-xs text-slate-500 leading-relaxed mb-6">
                El participante será eliminado de este evento y ya no aparecerá en las listas ni en las estadísticas.
              </p>
