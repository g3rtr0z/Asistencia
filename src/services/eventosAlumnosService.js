...
function mapFirestoreEventoAlumnoData(doc) {
  ...
  return {
    ...
    nombre: data.nombre || (data.nombres && data.apellidos ? `${data.nombres} ${data.apellidos}` : null),
    rut: data.rut,
...
export const agregarAlumnoAEvento = async (eventoId, alumno) => {
  try {
    const docRef = await addDoc(collection(db, COLLECTION_NAME), {
      ...
      nombre: alumno.nombre || `${alumno.nombres || ''} ${alumno.apellidos || ''}`.trim(),
      rut: alumno.rut,
      telefono: alumno.telefono || null,
      correo: alumno.correo || null,
      cargo: alumno.cargo || null,
      comuna: alumno.comuna || null,
